 ...
 * Class Segments
 */
class CustomerGroup extends AbstractEntity
{
    const ENTITY_CODE = 'customergroupimport';
    const TABLE = 'customer_group';
    const ENTITY_ID_COLUMN = 'customer_group_id';
    ...
    {
        $customerGroupId = (int) $rowData['customer_group_id'] ?? 0;
        $customerGroupCode = $rowData['customer_group_code'] ?? '';
        $taxClassId = (int) $rowData['tax_class_id'] ?? 0;

        if (!$customerGroupId) {
            $this->addRowError('CustomerGrouoIdIsRequired', $rowNum);
        }

        if (!$customerGroupCode) {
            $this->addRowError('CustomerGroupCodeIsRequired', $rowNum);
        }

        if (!$taxClassId) {
            $this->addRowError('TaxClassIdIsRequired', $rowNum);
        }
    ...
    private function initMessageTemplates()
    {
        $this->addMessageTemplate(
            'CustomerGrouoIdIsRequired',
            __('The Customer group id cannot be empty.')
        );
        $this->addMessageTemplate(
            'CustomerGroupCodeIsRequired',
            __('The Customer group code cannot be empty.')
        );
        $this->addMessageTemplate(
            'TaxClassIdIsRequired',
            __('The Tax class id cannot be empty.')
        );
    }
}
